...
<xsl:stylesheet xmlns:i18n="http://apache.org/cocoon/i18n/2.1"
    ...
    <xsl:template match="*[@n='comunity-browser']">
        <xsl:apply-templates select="./node()"/>
    </xsl:template>
    
    <xsl:template match="dri:div[@id='aspect.discovery.SimpleSearch.div.discovery-search-box']">
        <xsl:apply-templates select="*[@id!='aspect.discovery.SimpleSearch.div.general-query']"/>
    </xsl:template>

    <xsl:template match="//*[@id='aspect.eperson.PasswordLogin.div.register']">
        <!-- NO NECESITAMOS QUE SE MUESTRE NINGÚN ENLACE PARA REGISTRO -->
    </xsl:template>
